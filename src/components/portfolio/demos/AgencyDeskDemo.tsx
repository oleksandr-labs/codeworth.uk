...
export function AgencyDeskDemo({ lang }: { lang: string }) {
  ...
  return (
    <div className="h-screen bg-neutral-100 flex flex-col font-sans overflow-hidden">
      ...
      {/* ── HEADER ── */}
      <div className="shrink-0 bg-white dark:bg-neutral-800 border-b border-neutral-200 dark:border-neutral-700">
        {/* Top strip */}
        <div className="flex items-center justify-between px-5 py-2 border-b border-neutral-100 dark:border-neutral-700">
              ...
              return (
                <div key={col.id} className={`bg-white/70 rounded-2xl border-t-4 ${col.accent} flex flex-col overflow-hidden shadow-sm`}>
                  <div className="px-3 py-2.5 flex items-center justify-between shrink-0">
                    ...
                    <span className="text-[10px] text-neutral-400 bg-white dark:bg-neutral-800 rounded-full px-2 py-0.5">{colCards.length} · {colHours}h</span>
                  </div>
                  <div className="flex-1 overflow-y-auto px-2.5 pb-2.5 space-y-2">
                    {colCards.map(c => (
                      <div
                        ...
                        className={`cursor-pointer bg-white dark:bg-neutral-800 rounded-xl border p-3 group hover:shadow-md transition-shadow ${
                          c.id === activeTask ? "border-teal-400 ring-2 ring-teal-100" : "border-neutral-200 dark:border-neutral-700"
                        }`}
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-1">
                            <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${TAG_COLOUR[c.tag] ?? "bg-neutral-100 text-neutral-500 dark:text-neutral-400"}`}>{c.tag}</span>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={e => { e.stopPropagation(); move(c.id, -1); }} className="w-5 h-5 rounded bg-neutral-100 text-neutral-500 dark:text-neutral-400 text-xs hover:bg-neutral-200">←</button>
                              <button onClick={e => { e.stopPropagation(); move(c.id, 1); }} className="w-5 h-5 rounded bg-neutral-100 text-neutral-500 dark:text-neutral-400 text-xs hover:bg-neutral-200">→</button>
                            </div>
                          </div>
                        </div>
                    ...
                    {colCards.length === 0 && (
                      <div className="border-2 border-dashed border-neutral-200 dark:border-neutral-700 rounded-xl h-14 flex items-center justify-center text-[10px] text-neutral-300">
                        {isUk ? "Порожньо" : "Empty"}
                      </div>
                    )}
                  </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 overflow-hidden shadow-sm">
            <div className="px-5 py-3.5 border-b border-neutral-100 dark:border-neutral-700 flex items-center justify-between">
              <h3 className="font-bold text-neutral-900 dark:text-white text-sm">{isUk ? "Проєкти та виставлення рахунків" : "Projects & Invoicing"}</h3>
                ...
                <thead>
                  <tr className="bg-neutral-50 text-left">
                    {[
                      ...
                      <th key={i} className="px-4 py-2.5 text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={i} className="hover:bg-neutral-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="font-semibold text-neutral-900 dark:text-white">{row.client}</div>
                        ...
                        <td className="px-4 py-3 font-mono text-neutral-600 dark:text-neutral-300">£{row.rate}/h</td>
                        <td className="px-4 py-3">
                          <div className="font-bold text-neutral-900 dark:text-white">£{totalValue.toLocaleString()}</div>
                          {unbilledHours > 0 && (
            ...
            {/* Summary footer */}
            <div className="px-5 py-3 bg-neutral-50 border-t border-neutral-100 dark:border-neutral-700 flex items-center justify-between">
              <span className="text-xs text-neutral-500 dark:text-neutral-400">{isUk ? "Всього до виставлення цього місяця" : "Total unbilled this month"}</span>
      ...
      {tab === "team" && (
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-neutral-900 dark:text-white">{isUk ? "Завантаженість команди — цей тиждень" : "Team utilisation — this week"}</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {TEAM_DATA.map(member => {
              ...
              const cardBorder = isOver ? "border-red-200 bg-red-50" : "border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800";
                ...
                <div key={member.id} className={`rounded-2xl border p-5 shadow-sm ${cardBorder}`}>
                  ...
                  <div className="mb-3">
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-neutral-500 dark:text-neutral-400">{isUk ? "Завантаженість" : "Utilisation"}</span>
                  ...
                  {/* Current task */}
                  {task && (
                    <div className="flex items-start gap-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2">
          ...
          <div className="max-w-2xl mx-auto space-y-4">
            ...
            <div className="bg-amber-50 rounded-2xl border border-amber-200 p-5">
              ...
              <div className="flex gap-2">
                ...
                <button className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-300 text-sm px-4 py-2 rounded-xl hover:bg-neutral-50 transition-colors">{isUk ? "Запросити правки" : "Request changes"}</button>
              </div>
            </div>
                ...
                {[
                  ...
                  <div key={i} className="flex items-center justify-between py-2.5">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-neutral-900 dark:text-white">{inv.amount}</span>
                      {inv.status === "paid" ? (
          ...
          <>
            ...
            <aside className="absolute top-0 right-0 bottom-0 w-full max-w-sm bg-white dark:bg-neutral-800 z-40 shadow-2xl flex flex-col overflow-hidden">
              ...
              <div className="px-5 py-3 border-t border-neutral-100 dark:border-neutral-700 flex gap-2">
                ...
                <button onClick={() => setDetailId(null)} className="px-4 text-sm text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 rounded-xl transition-colors">{isUk ? "Закрити" : "Close"}</button>
              </div>
            </aside>
          </>
        );
