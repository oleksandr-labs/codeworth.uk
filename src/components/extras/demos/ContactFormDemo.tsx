      ...
      <div className="max-w-xl mx-auto bg-blue-50 rounded-3xl p-8">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Ім'я *" : "Name *"}</label>
              <input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))}
                type="text" placeholder={isUk ? "Ваше ім'я" : "Your name"}
                className="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-white focus:border-blue-400 focus:outline-none text-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Телефон *" : "Phone *"}</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Послуга" : "Service"}</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Бюджет" : "Budget"}</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Повідомлення" : "Message"}</label>
            <textarea value={form.message} onChange={(e) => setForm((p) => ({ ...p, message: e.target.value }))}
              rows={3} placeholder={isUk ? "Коротко про ваш проєкт..." : "Brief about your project..."}
              className="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-white focus:border-blue-400 focus:outline-none text-sm resize-none" />
          </div>
  ...
  return (
    <div>
      ...
      <div className="max-w-xl mx-auto bg-slate-50 rounded-3xl p-8">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-neutral-600 mb-2">{isUk ? "Тип нерухомості" : "Property type"}</label>
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <label key={label} className="flex items-center gap-2 p-3 bg-white rounded-xl border border-neutral-200 cursor-pointer hover:border-blue-300">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Бюджет (₴)" : "Budget (UAH)"}</label>
              <select className="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-white focus:border-blue-400 focus:outline-none text-sm">
            ...
            <div>
              <label className="block text-xs font-semibold text-neutral-600 mb-1.5">{isUk ? "Площа (м²)" : "Area (m²)"}</label>
              <select className="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-white focus:border-blue-400 focus:outline-none text-sm">
                <option>до 50</option>
                <option>50–80</option>
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-600 mb-2">{isUk ? "Бажані райони" : "Preferred districts"}</label>
            <div className="flex flex-wrap gap-2">
              {(isUk ? ["Шевченківський", "Печерський", "Подільський", "Голосіївський", "Оболонський"] : ["Shevchenkivsky", "Pechersky", "Podilsky", "Holosiivsky", "Obolon"]).map((d) => (
                <label key={d} className="flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-full border border-neutral-200 cursor-pointer hover:border-blue-400 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="text" placeholder={isUk ? "Ваше ім'я" : "Your name"}
              className="px-4 py-3 rounded-xl border border-neutral-200 bg-white focus:border-blue-400 focus:outline-none text-sm" />
