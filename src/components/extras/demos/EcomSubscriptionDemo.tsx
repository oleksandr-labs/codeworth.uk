  ...
  if (stage === "plans") {
    return (
      <div className="space-y-6">
        <div>
          <h3 className="text-xl font-bold text-neutral-900 mb-1">{isUk ? "Оберіть план підписки" : "Choose your subscription plan"}</h3>
          ...
          {PLANS.map((p) => (
            <button
              ...
              className={`text-left rounded-2xl border-2 p-5 transition-all relative ${
                selected === p.id ? "border-indigo-500 bg-indigo-50/30 shadow-lg" : "border-neutral-200 bg-white hover:border-neutral-300"
              }`}
            >
              {p.popular && (
              ...
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-3xl font-extrabold text-neutral-900 tabular-nums">{fmt(p.price)}</span>
        ...
        <div className="rounded-xl bg-neutral-50 border border-neutral-200 p-4 space-y-2 text-sm">
          {(isUk
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
          <div>
            <p className="text-xs text-neutral-500 uppercase tracking-wider mb-1">{isUk ? "Статус" : "Status"}</p>
          ...
          <div>
            <p className="text-xs text-neutral-500 uppercase tracking-wider mb-1">{isUk ? "Метод оплати" : "Payment method"}</p>
            <p className="font-bold text-neutral-900 tabular-nums">•••• 4242</p>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-neutral-100 flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-neutral-200 text-sm text-neutral-700 hover:bg-neutral-50"
          >
            {status === "active" ? (
              <>
          ...
          <button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-neutral-200 text-sm text-neutral-700 hover:bg-neutral-50">
