...
export function MultiStepFormDemo({ isUk }: Props) {
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      {step === 0 && (
        <div>
          <h2 className="text-xl font-heading font-extrabold text-neutral-900 mb-6">{t.s1title}</h2>
            ...
            {services.map((s) => (
              <button
                ...
                className={`flex items-center gap-3 w-full px-4 py-3.5 rounded-2xl border-2 text-left transition-all ${
                  service === s.id ? "border-indigo-500 bg-indigo-50" : "border-neutral-200 bg-white hover:border-indigo-300"
                }`}
              >
                <span className="text-xl">{s.icon}</span>
                <span className="font-semibold text-sm text-neutral-900 flex-1">{s.label}</span>
      ...
      {step === 1 && (
        <div>
          <h2 className="text-xl font-heading font-extrabold text-neutral-900 mb-6">{t.s2title}</h2>
          <div className="space-y-5">
            <div>
              <label className="block text-sm font-semibold text-neutral-700 mb-2">{t.budgetLabel}</label>
                ...
                {budgets.map((b) => (
                  <button
                    ...
                    className={`px-3 py-2.5 rounded-xl border-2 text-xs font-semibold text-left transition-all ${
                      budget === b ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-neutral-200 bg-white hover:border-indigo-300 text-neutral-700"
                    }`}
                  >
                    {b}
                  </button>
            ...
            <div>
              <label className="block text-sm font-semibold text-neutral-700 mb-2">{t.deadlineLabel}</label>
              <div className="grid grid-cols-2 gap-2">
                {deadlines.map((d) => (
                  <button
                    key={d}
                    ...
                    className={`px-3 py-2.5 rounded-xl border-2 text-xs font-semibold text-left transition-all ${
                      deadline === d ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-neutral-200 bg-white hover:border-indigo-300 text-neutral-700"
                    }`}
                  >
                    {d}
                  </button>
      ...
      {step === 2 && (
        <div>
          <h2 className="text-xl font-heading font-extrabold text-neutral-900 mb-6">{t.s3title}</h2>
          <div className="space-y-4">
            ...
            {/* Summary */}
            <div className="p-4 rounded-2xl bg-neutral-50 border border-neutral-100 text-xs space-y-1.5">
              <p className="font-semibold text-neutral-600 mb-2">{isUk ? "Ваш вибір:" : "Your selection:"}</p>
      ...
      {step === 3 && submitted && (
        <div className="text-center py-8">
          ...
          <h2 className="text-2xl font-heading font-extrabold text-neutral-900 mb-2">{t.successTitle}</h2>
          <p className="text-neutral-600 mb-1">{t.successText}</p>
          ...
          <button
            ...
            className="mt-4 text-xs text-neutral-500 hover:text-neutral-700 underline underline-offset-2 transition-colors"
          >
