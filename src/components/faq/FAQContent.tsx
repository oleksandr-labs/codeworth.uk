    ...
    <>
      ...
      <div className="max-w-xl mx-auto mb-12">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-neutral-200 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all placeholder:text-neutral-400 text-neutral-900"
          />
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto space-y-16">
        {searchResults !== null ? (
          ...
          searchResults.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {searchResults.map((item, i) => (
                <details
                  key={i}
                  className="group p-6 rounded-2xl border border-neutral-100 open:border-indigo-100 open:shadow-sm transition-all"
                >
                  <summary className="flex items-start justify-between gap-4 cursor-pointer list-none">
                    <div>
                      <span className="text-xs text-indigo-500 font-medium mb-1 block">{item.section}</span>
                      <h3 className="font-heading font-bold text-neutral-900 group-open:text-indigo-700 transition-colors">
                        {item.q}
                      </h3>
                    </div>
                  ...
                  <p className="mt-4 text-neutral-600 text-sm leading-relaxed">{item.a}</p>
                </details>
              ))}
            </div>
          )
          ...
          sections.map((section) => (
            <div key={section.id} id={section.id}>
              <h2 className="text-2xl font-heading font-extrabold text-neutral-900 mb-6 pb-3 border-b border-neutral-100">
                {section.title}
              </h2>
              <div className="space-y-4">
        ...
        <div className="p-8 rounded-2xl bg-indigo-50 border border-indigo-100 text-center">
          ...
          <h3 className="text-xl font-heading font-bold text-neutral-900 mb-2">{isUk ? "Не знайшли відповідь?" : "Didn't find an answer?"}</h3>
