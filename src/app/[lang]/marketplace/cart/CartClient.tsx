...
export default function CartClient() {
  ...
  if (!isHydrated) {
    return (
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-6 space-y-4">
        {[1, 2].map((i) => (
          <div key={i} className="flex gap-4 items-start p-4 rounded-xl border border-gray-100 dark:border-neutral-800">
  ...
  if (items.length === 0) {
    return (
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-12 text-center">
        ...
        <p className="text-gray-500 mb-6">{isUk ? "Додайте готові рішення з нашого каталогу" : "Add ready-made solutions from our catalog"}</p>
        <Link
  ...
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="bg-white rounded-2xl border border-gray-100 p-5 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-gray-500 mt-0.5">{isUk ? "Пакет" : "Package"}: {PACKAGE_DISPLAY[item.package]?.[isUk ? "uk" : "en"] ?? item.package}</div>
          ...
          <div className="text-right shrink-0">
            ...
            <button
              ...
              className="mt-2 p-1.5 text-gray-400 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50"
            >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <p className="text-xs text-gray-400 mt-2">{isUk ? "Спробуйте: CODENEST10" : "Try: CODENEST10"}</p>
      </div>
