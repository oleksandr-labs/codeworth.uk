            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="grid grid-cols-2 gap-4">
                {VALUES.map((v) => {
                  const Icon = v.icon;
                  return (
                    <div key={v.title} className="p-5 rounded-2xl bg-neutral-50 border border-neutral-100">
                      <div className="w-10 h-10 rounded-xl bg-indigo-100 flex items-center justify-center mb-3">
                        <Icon className="w-5 h-5 text-indigo-600" />
                      </div>
                      <h3 className="font-heading font-bold text-neutral-900 mb-1 text-sm">{v.title}</h3>
        ...
        <section className="py-24 bg-white">
          <Container>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              {TEAM.map((member) => (
                <div key={member.name} className="text-center p-6 rounded-2xl border border-neutral-100 hover:shadow-md transition-shadow duration-200">
                  ...
                  <h3 className="font-heading font-bold text-neutral-900 mb-0.5">{member.name}</h3>
                  <p className="text-sm text-indigo-600 font-medium mb-1">{member.role}</p>
                  <p className="text-xs text-neutral-500 leading-relaxed mb-3">{member.bio}</p>
                  <p className="text-xs text-neutral-400 mb-3">{member.skills}</p>
                  <div className="flex items-center justify-center gap-2">
                    {member.linkedin && (
                      <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="w-7 h-7 rounded-lg bg-neutral-100 hover:bg-indigo-100 flex items-center justify-center text-neutral-500 hover:text-indigo-600 transition-colors" aria-label={`LinkedIn ${member.name}`}>
                        <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                      </a>
                    )}
                    {member.github && (
                      <a href={member.github} target="_blank" rel="noopener noreferrer" className="w-7 h-7 rounded-lg bg-neutral-100 hover:bg-neutral-200 flex items-center justify-center text-neutral-500 hover:text-neutral-900 transition-colors" aria-label={`GitHub ${member.name}`}>
                    ...
                    {member.dribbble && (
                      <a href={member.dribbble} target="_blank" rel="noopener noreferrer" className="w-7 h-7 rounded-lg bg-neutral-100 hover:bg-pink-100 flex items-center justify-center text-neutral-500 hover:text-pink-600 transition-colors" aria-label={`Dribbble ${member.name}`}>
            ...
            <div className="p-8 rounded-2xl bg-linear-to-br from-indigo-50 to-violet-50 border border-indigo-100 text-center">
              ...
              <p className="text-neutral-600 mb-4 max-w-lg mx-auto text-sm">
                {isUk
