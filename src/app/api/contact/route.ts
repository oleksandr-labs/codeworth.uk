...
export async function POST(request: NextRequest) {
  // Parse first so the rate-limit/error responses below can be localized
  ...
  try {
    ...
    if (body.recaptchaToken) {
      const isHuman = await verifyRecaptcha(body.recaptchaToken as string, "contact");
      if (!isHuman) {
        return NextResponse.json(
          { error: localizeApiMessage("Перевірка безпеки не пройдена. Спробуйте ще раз.", lang) },
          { status: 400 }
        );
      }
    }
    ...
    if (resendKey) {
      const html = lang === "en" ? `<!DOCTYPE html>
...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f4f4f5;">
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    ...
    <div style="padding:36px;">
      <h2 style="color:#1E1B4B;font-size:18px;margin:0 0 12px;">We've received your request ✅</h2>
      <p style="color:#374151;font-size:14px;line-height:1.7;margin:0 0 20px;">
        Hi <strong>${payload.name}</strong>! Thank you for reaching out to Codeworth.
        We'll be in touch within <strong>2 hours</strong>.
      </p>
      ${payload.service ? `<p style="color:#374151;font-size:14px;margin:0 0 8px;">📋 <strong>Service:</strong> ${payload.service}</p>` : ""}
      ${payload.budget ? `<p style="color:#374151;font-size:14px;margin:0 0 20px;">💰 <strong>Budget:</strong> ${payload.budget}</p>` : ""}
...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f4f4f5;">
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
      ...
      try {
        await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            subject: lang === "en" ? "Request received — Codeworth will be in touch soon" : "Заявку прийнято — Codeworth зв'яжеться з вами скоро",
            html,
          }),
        });
      } catch {
    ...
    return NextResponse.json(
      { success: true, message: localizeApiMessage("Заявку отримано. Зв'яжемося протягом 2 годин!", lang) },
      { status: 200 }
    );
  } catch (err) {
    if (err instanceof ZodError) {
      return NextResponse.json(
        { error: localizeApiMessage(err.issues[0]?.message ?? "Некоректні дані", lang) },
        { status: 400 }
      );
    }
    return NextResponse.json(
      { error: localizeApiMessage("Щось пішло не так. Спробуйте ще раз або напишіть нам у Telegram.", lang) },
      { status: 500 }
    );
  }
}
