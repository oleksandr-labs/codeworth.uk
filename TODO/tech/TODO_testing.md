> 🔴 **НОВА ЗНАХІДКА (2026-07-12, 5-й прохід аудиту) — E2E-покриття programmatic-роутів фактично 0%, і 2 наявні e2e-тести застарілі/биті.**
>
> Перевірено напряму: `e2e/` містить лише 7 spec-файлів (`contact`, `home`, `i18n`, `seo`, `smoke`, `viewport`, `visual`). **Жоден з них не тестує `compare/[slug]`, `location/[city]`, `glossary/[term]`, `portfolio/[slug]`, `tools/[slug]`, `startup/[slug]`, `careers/[position]`, `resources/[slug]`, `use-cases/[slug]`, `ai/[niche]`, `ml/[niche]`** — тобто увесь programmatic-шар сайту (переважна більшість з ~1000+ сторінок: 286 blog + 449 glossary + 71 compare + 101 portfolio + 44 geo + 26+18 ніш + інше) не має жодного e2e-тесту, що завантажує реальну сторінку в браузері. Jest-тести (`sitemap.test.ts`) перевіряють лише що масиви даних коректно потрапляють у sitemap-генерацію, не що сторінки реально рендеряться без помилок.
> - [ ] Знайдено 2 конкретні застарілі/биті e2e-тести:
>   - `e2e/seo.spec.ts` (рядки 168, 189) — `page.goto("/blog/next-js-seo-guide-2024")` — **цей slug відсутній у `blog.ts`** (перевірено `grep`, 0 збігів) — вочевидь legacy web-studio-ери пост, видалений при ML-ребрендингу; тест зараз або 404-иться, або (якщо CI не падає) мовчки не тестує те, що мав
>   - `e2e/smoke.spec.ts` (рядок 24, тест `"маркетплейс каталог завантажується"`) — `page.goto("/marketplace/catalog")` — маршрут видалено в Sprint 2026-07-02 (`TODO_REMOVE_LEGACY_ECOMMERCE.md`); URL зараз 301-редіректить на `/use-cases` (`next.config.ts`) — тест технічно "проходить" (бачить якийсь `h1`, 0 JS-помилок), але тестує НЕ те, що написано в назві тесту (маркетплейс не існує), а побічно — що редірект-таргет `/use-cases` рендериться без помилок. Назва тесту вводить в оману
> - [ ] **Дія:** (1) видалити/переписати `e2e/smoke.spec.ts` тест на реальну перевірку `/use-cases` під власною назвою; (2) замінити мертвий slug у `e2e/seo.spec.ts` на реальний, напр. `what-is-mlops` чи будь-який актуальний ML-пост; (3) додати мінімум по 1 smoke e2e-тесту на кожен programmatic-тип (`compare/[якийсь-slug]`, `location/[якесь-місто]`, `glossary/[якийсь-term]` тощо) — перевірка H1 присутній, 0 JS errors, JSON-LD валідний
> - [ ] Пов'язано з [seo/TODO_technical_seo.md](../seo/TODO_technical_seo.md) критичною знахідкою — якби існував e2e-тест на `/location/[city]`, він би одразу впіймав redirect-баг (P0 у `TODO_ROADMAP.md`), бо тест отримав би 301 замість очікуваної сторінки. Відсутність е2е-покриття напряму дозволила цьому багу лишитись непоміченим.
...
**Статус:** Частково виконано (unit тести — **972 passed, 87 suites** — 2026-07-09; E2E конфіг + тести написані; 66 a11y тести / 25 компонентів)
**✅ Оновлено 2026-07-09 (пост-ML-ребрендинг тестовий аудит):**
- Усі 20 тестових файлів (94 тести), що ще звірялись зі старим web-studio/"CodeNest" брендингом замість актуального ML/AI-контенту Codeworth, оновлено під поточні компоненти/дані. Файли: `HomeHero`, `HomeSections` (ClientLogosSection/HowWeWorkSection/TestimonialsSection/IndustriesTeaser/BlogPreviewSection), `CasesSection`, `CTASection`, `HeroAndBlog`, `HomeComponents`, `Logo`×2, `ThemeToggle`, `ThemeProvider`, `FloatingChat`, `ExtraCard`, `ExtrasHero`, `BlogContent`, `PricingContent`, `PriceCalculator`, `ContactForm`, а також lib data-тести `blog.test.ts`, `services.test.ts`, `extras.test.ts`, `portfolio.test.ts`.
- Разом з цим знайдено й виправлено 2 реальні дрібні дефекти в продакшн-даних (не в тестах): `src/lib/data/blog.ts` — 12 постів мали `color: "emerald"` (і ще 7 інших однослівних значень типу `"amber"`/`"indigo"`) замість градієнтного класу `"from-X-500 to-Y-600"`, через що фон featured/list-карток блогу рендерився без градієнта; `src/lib/data/portfolio.ts` — `NICHES`/`CATEGORIES` мали `"Всi"` з латинською `i` замість кириличної `і`.
- Підсумок: 972/972 тестів (87 suites) зелені, `tsc --noEmit` 0 помилок, `npm run build` успішний.
- Сам сайт та ще пара суміжних TODO (sitemap-баг, 5 "відсутніх" сторінок послуг, WebsiteCostCalculator.tsx) виявились вже фактично полагодженими раніше — ці пункти в `TODO_MAIN.md` застаріли, самого дефекту не було.

**✅ Оновлено 2026-07-09 (ESLint):** {#eslint}
- `npm run lint` (=`eslint .`, викликається в `.github/workflows/ci.yml` без `continue-on-error`) повертав exit 1 через 73 помилки — CI лінт-гейт був червоним для будь-якого PR. Виправлено всі 73; лишилось 61 warning (unused vars/imports), вони не блокують `eslint .` (exit 0).
- Реальні баги, знайдені лінтером (не просто стиль): `Math.random()` викликався прямо в JSX під час рендеру `TelegramBotDemo` (`src/components/extras/demos/GenericDemo.tsx`) — номер заявки #XXXX змінювався на кожен ре-рендер (напр. після кліку Accept/Reject); виправлено генерацією номера в `onClick` з фіксацією в `useState`. Мутація змінної `cumulative` під час `.map()` у SVG donut-chart рендері (`RetailCoreDemo.tsx`) — замінено на precomputed `CATEGORY_CUMULATIVE` масив поза компонентом. Компоненти `BeforeContent`/`AfterContent` створювались заново на кожен рендер всередині `BeforeAfterDemo` (`react-hooks/static-components`) — перетворено на прості JSX-змінні (`beforeContent`/`afterContent`).
- Обґрунтовані `eslint-disable` (не помилки коду, а свідомі винятки з поясненням у коментарі): `global-error.tsx` використовує `<a href="/">` замість `next/link` — навмисний патерн Next.js, бо цей файл замінює весь root layout при критичному збої і не може покладатись на router-контекст, який теж міг впасти; `ThemeProvider.tsx` читає `localStorage`/`matchMedia` синхронно в ефекті при маунті (`react-hooks/set-state-in-effect`) — не можна зробити лениво через `useState(() => ...)` (SSR не має `localStorage`), а відкладення через `queueMicrotask` ламало синхронні тести й додавало непотрібний "спалах" теми; `ChainOpsDemo.tsx` — `Math.random()` у `logWaste()`, викликається лише з `onClick`, ніколи під час рендеру (false positive статичного аналізу).
- Механічно виправлено (~45 помилок): unescaped `'`/`"` у JSX-тексті (`&apos;`/`&ldquo;`/`&rdquo;`) в `privacy/page.tsx`, `terms-of-service/page.tsx`, `showcase/*`, `use-cases/[slug]/page.tsx`, кількох startup/extras demo-компонентах і 2 тестових файлах; `@typescript-eslint/no-explicit-any` у `HomeHero.test.tsx` (3 місця) замінено на конкретні типи; `NavProgress.tsx` — той самий `set-state-in-effect`, але тут виправлено по-справжньому (обгорнуто в `setTimeout(...,0)` замість disable-коментаря).
- Перевірка після фіксу: `eslint .` exit 0, 972/972 тестів зелені, `tsc --noEmit` 0 помилок, `npm run build` успішний (усі роути згенеровано).
