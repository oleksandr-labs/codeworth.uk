...
- [ ] Review/Rating — після накопичення реальних відгуків. ⚠️ **Перевірено 2026-07-09**: `src/lib/data/reviews.ts` містить лише вигадані маркетингові приклади відгуків (`verified: true`, але автори/компанії не реальні) — НЕ використовувати ці дані для `Review`/`AggregateRating` schema, це порушує Google policy щодо fake review markup. Деталі: `TODO/pages/services/TODO_services_overview.md`.
  - ✅ **Повторно верифіковано 2026-07-12 (3-й прохід):** `grep -rho '"@type": "[A-Za-z]*"' src/` — **0 збігів** `Review`/`AggregateRating` у всьому `src/` — підтверджено, що навмисне рішення НЕ підключати `reviews.ts` до schema досі дотримується коду. Повний інвентар JSON-LD-типів, реально присутніх у коді (підрахунок входжень `"@type"`, 2026-07-12): `ListItem` 67, `BreadcrumbList` 26, `Organization` 23, `Offer` 15, `Question`/`Answer` по 12, `FAQPage` 10, `PostalAddress` 7, `Service` 6, `Person` 5, `HowToStep` 5, `ItemList`/`CollectionPage` по 4, `SoftwareApplication`/`QuantitativeValue`/`ProfessionalService`/`Article` по 3, `WebSite`/`SearchAction`/`Product`/`PriceSpecification`/`Place`/`OfferCatalog`/`MonetaryAmount`/`JobPosting`/`ImageObject`/`HowTo`/`EntryPoint`/`DefinedTermSet`/`Country`/`Brand` по 2, і 8 типів по 1 (`WebPage`, `WebApplication`, `Thing`, `TechArticle`, `OpeningHoursSpecification`, `HowToSection`, `DefinedTerm`, `ContactPoint`, `City`, `BlogPosting`). `application/ld+json` рендериться на 36 файлах `page.tsx`. Дивись [seo/TODO_hreflang.md](TODO_hreflang.md) розділ "JSON-LD мовна локалізація" — сама наявність schema підтверджена широка, але текст усередині кількох типів (`Article.headline`, `BreadcrumbList` на blog/portfolio/glossary) хардкоджений українською незалежно від локалі, це окрема від "яких типів бракує" проблема.
