> ⚠️ **DEPRECATED (2026-07-12) — не використовувати як актуальне джерело.** Весь цей документ описує до-ML позиціонування бренду "CodeNest" з живим маркетплейсом (кошик/чекаут/LiqPay), сторінкою `/niches` та ідеєю "Niches → Solutions" рібрендингу. Ця інфраструктура **повністю видалена** в Sprint 2026-07-02 (324 файли, ~151k рядків — див. [TODO_REMOVE_LEGACY_ECOMMERCE.md](TODO_REMOVE_LEGACY_ECOMMERCE.md)), а контент переписано під ML/AI-консалтинг (`TODO_ML_ADAPTATION.md`, Sprints 6–69). Актуальна модель монетизації — ML-проєкти (PoC/Production/Enterprise) + MLOps retainer, без маркетплейсу готових сайтів. Розділ 9 ("Прибирання фейкових клієнтських претензій") лишається частково релевантним як історичний контекст — див. актуальний **[seo/TODO_trust_authenticity.md](seo/TODO_trust_authenticity.md)** для поточного стану цієї проблеми (та сама категорія ризику досі не повністю закрита, тепер у `reviews.ts`/`portfolio.ts`, не в marketplace-відгуках).
